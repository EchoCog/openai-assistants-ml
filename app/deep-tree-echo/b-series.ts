...
import { RecursiveImbrication } from './recursive-imbrication';
...
export class BSeriesIntegrator {
  ...
  analyzeDimensionalStructure(level: number): {
    spatialAnalysis: string;
    incidenceStructure: string;
    discrepancyAnalysis?: string;
  } {
    return {
      spatialAnalysis: DimensionalPatterns.analyzePattern(level),
      incidenceStructure: DimensionalPatterns.visualizeIncidenceStructure(level),
      discrepancyAnalysis: DimensionalPatterns.explainDiscrepancy(level)
    };
  }

  mapTreeToGeometric(tree: BSeriesTree): string {
    const level = this.calculateTreeLevel(tree);
    const pattern = DimensionalPatterns.analyzePattern(level);
    return pattern;
  }

  private calculateTreeLevel(tree: BSeriesTree): number {
    let level = 1;
    let current = tree;
  ...
  analyzeRecursiveStructure(tree: BSeriesTree): {
    imbrication: string;
    unity: string;
    projective: RecursiveLayer[];
  } {
    const depth = this.calculateTreeDepth(tree);
    const pattern = RecursiveImbrication.generateImbrication(depth);
    
    return {
      imbrication: RecursiveImbrication.analyzeImbrication(pattern),
      unity: RecursiveImbrication.analyzeUnityEmergence(),
      projective: RecursiveImbrication.mapProjectiveScaling(2, depth) // Using 2 as base for Unity emergence
    };
  }

  private calculateTreeDepth(tree: BSeriesTree): number {
    const sequence = this.extractSequence(tree);
    return RecursiveImbrication.calculateRecursiveDepth(sequence);
  }

  private extractSequence(tree: BSeriesTree): number[] {
    const sequence: number[] = [];
    let current: BSeriesTree | null = tree;
    
    while (current) {
      sequence.push(this.calculateNodeValue(current));
  ...
  private calculateNodeValue(node: BSeriesTree): number {
    // Calculate value based on node properties
    let value = 1; // Base value
    
    // Add value for children
    if (node.children) {
      value += node.children.length;
    }
    
    // Add value for properties/attributes
    if (node.properties) {
